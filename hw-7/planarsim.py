...
# normalize a homogenous coordinate point, if last element non-zero
def normalize_homog(point):
    p = list(point)
    if p[2] == 0:
        return p
...
# computes rotation center locations in homogenous coordinates,
# in the frame of the robot. Rotation centers may be at infinity
def compute_rframe_rcs(controls):
    rc_list = []
    for u in controls:
        rc = normalize_homog([-u[1], u[0], u[2]])
        rc_list.append(rc)
...
# computes rotation center locations in homogenous coordinates,
# in the world frame, assuming robot configuration q
def compute_worldframe_rcs(controls, q):
    rframe_rcs = compute_rframe_rcs(controls)
    T = transform_from_config(q)
    for i in range(len(rframe_rcs)):
        rframe_rcs[i] = T @ rframe_rcs[i]
